...
def rnn_step_backward(dnext_h, cache):
    ...
    diff_tanh=(1-(next_h)**2)
    dnext_diff=dnext_h*diff_tanh

    dx=np.dot(dnext_diff,Wx.T)
    dprev_h=np.dot(dnext_diff,Wh.T)

    dWx=np.dot(x.T,dnext_diff)
    dWh=np.dot(prev_h.T,dnext_diff)
    db=np.sum(dnext_diff,axis=0)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
        ...
        tx, tprev_h, tWx, tWh, tb=rnn_step_backward(dh[:,t,:]+tprev_h, cache[t])
        #addition is there because if the pathway splits during the forward prop then during backprop they come back and add up.
        #dh has values of all the hidden state and the prev_hidden state calculated by step backward are added. 
...
def lstm_backward(dh, cache):
    ...
    x, Wx, Wh, b, prev_h, prev_c, i, f, o, g, next_c, next_h =cache[0]
    N,T,H=dh.shape
    N,D=cache[0][0].shape
    ...
    for t in reversed(range(T)):
        ...
        dx0, dprev_h, dprev_c, dWx0, dWh0, db0=lstm_step_backward(dh[:,t,:]+dprev_h, dprev_c, cache[t])
