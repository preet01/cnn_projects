...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache=mu,xmu,sq,var,sqrtvar,ivar,xhat,gammax,out,gamma,eps
...
def batchnorm_backward(dout, cache):
    ...
    N,D=dout.shape
    mu,xmu,sq,var,sqrtvar,ivar,xhat,gammax,out,gamma,eps=cache    
   
    dbeta=np.sum(dout,axis=0)#9
    dgammax=dout#9

    dgamma=np.sum(dgammax*xhat,axis=0)#8
    dxhat=gamma*dgammax#8

    dxhat_xmu=ivar*dxhat#7
    divar = np.sum(dxhat*xmu, axis=0)#7#Check
    
    dsqrtvar=-1. /(sqrtvar**2) * divar#6

    dvar=dsqrtvar/(2*np.sqrt(var+eps))#5

    dsq=1./N*np.ones((N,D))*dvar#4

    dsq_xmu=2*xmu*dsq#3

    dxmu_x=1*(dxhat_xmu+dsq_xmu)#2
    dxmu_mu=-1*np.sum(dxhat_xmu+dsq_xmu,axis=0)#2#check

    dmu_x=1./N*np.ones((N,D))*dxmu_mu#1

    dx=dmu_x+dxmu_x#check
       ...
       for cc in range(C):
           ...
           for ii in range(0,out_r):
               ...
               for jj in  range(0,out_c):
                  ...
                  out[nn,cc,ii,jj]=np.amax(x[nn, cc, ii*s:ii*s+hh, jj*s:jj*s+ww])     
       ...
       for cc in range(C):
           ...
           for ii in range(0,out_r):
               ...
               for jj in  range(0,out_c):
                  ...
                  max_pos=np.argmax(x[nn, cc, ii*s:ii*s+hh, jj*s:jj*s+ww])     
                  
                  max_coord=np.unravel_index(max_pos,(hh,ww))
              
                  dx[nn, cc, ii*s:ii*s+hh, jj*s:jj*s+ww][max_coord]=dout[nn,cc,ii,jj]     #in dx matrix size given selects max_coord 
...
def spatial_batchnorm_backward(dout, cache):
    ...
    #print(dout_flat.shape)
 
    dx,dgamma,dbeta =batchnorm_backward(dout_flat, cache)

    dx_reshaped = dx.reshape(*dout.shape)
